...
`setup` asks for a key from [console.openmail.sh](https://console.openmail.sh) (any scope), picks or creates the inbox, swaps a broad key for an inbox-scoped one before writing `~/.hermes/.env`, and opens Hermes's sender gate (who actually gets through is OpenMail policy, see below). `hermes openmail doctor` checks the result. Flags: `--api-key`, `--api-key-stdin`, `-y` for scripts.
